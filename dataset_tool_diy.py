...
# 1024 x 704
def create_skirt_wide(tfrecord_dir, lmdb_dir, width=704, height=1024, max_images=None):
    # raw = create_lsun_wide(tfrecord_dir, lmdb_dir, width=512, height=384, max_images=None)
    create_lsun_wide(tfrecord_dir, lmdb_dir, width=width, height=height, max_images=max_images)
#----------------------------------------------------------------------------
...
def create_from_images(tfrecord_dir, image_dir, shuffle):
    ...
    img = np.asarray(PIL.Image.open(image_filenames[0]).convert('RGB').resize([512, 512])) # $$
    resolution = img.shape[0]
    channels = img.shape[2] if img.ndim == 3 else 1
    if img.shape[1] != resolution:
        error('Input images must have the same width and height')
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames)) as tfr:
        ...
        for idx in range(order.size):
            #img = np.asarray(PIL.Image.open(image_filenames[order[idx]]))
            img = np.asarray(PIL.Image.open(image_filenames[order[idx]]).convert('RGB').resize([512, 512]))  # $$
            if channels == 1:
                img = img[np.newaxis, :, :] # HW => CHW
            else:
                img = img.transpose([2, 0, 1]) # HWC => CHW
